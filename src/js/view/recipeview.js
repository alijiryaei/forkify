...
class RecipeView extends View {
  ...
  addEventHandler(input) {
    window.addEventListener('hashchange', input);
  ...
  addServingsHandler(handler) {
    this._parentElement.addEventListener('click' , function(e){
      ...
      if(!btn) return;
      const newServ = +btn.dataset.servings;
  ...
  _generateMarkupIngredient(ent) {
    return `<li class="recipe__ingredient">
    <svg class="recipe__icon">
      <use href="${icons}#icon-check"></use>
    </svg>
    <div class="recipe__quantity">${
      ent.quantity ? new Fraction(ent.quantity).toString() : ''
    }</div>
    <div class="recipe__description">
      <span class="recipe__unit">${ent.unit}</span>
      ${ent.description}
    </div>
    </li>`;
  }
